...
function VerifyContent() {
    ...
    const executeLookup = async (targetHash: string) => {
        ...
        `)
                .eq('sha256_hash', targetHash.trim())
                .maybeSingle();
            ...
            setErrorMsg(err.message || 'Verification lookup failed. Check Supabase connection.');
        } finally {
            setVerifying(false);
        }
    };
    ...
    const handleManualVerify = async () => {
        ...
        if (file) {
            try {
                ...
                setErrorMsg('Error calculating file hash: ' + e.message);
                setVerifying(false);
                return;
            }
        }
        ...
        await executeLookup(targetHash);
    };
    ...
    return (
        <div className="max-w-xl w-full p-6 bg-white rounded-xl shadow-md border border-gray-200">
            <h2 className="text-lg font-bold mb-4 text-gray-900">Check Provenance</h2>
            ...
            <button
                ...
                disabled={verifying || (!file && !manualHash.trim())}
